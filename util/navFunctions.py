...
import hardware.imu_comm_new as imu #used to communicate with the IMU
import threading #used to make threads
from otherFunctions import * #populate name space with otherFunctions code
from simulation.ins_sim import * #populate name space with ins_sim code
...
class kalmanControl(object):
    def __init__(self,fs = 100.0,leverArm = np.zeros((3,1)),measNoise = np.zeros((15,15)), sim = None, simInit = None):
        ################# Constants #########################
        #sample rate
        self.fs = fs
        self.ts = 1/self.fs
        ...
        if sim is None:
            ...
            # NOTE:
            '''
            Sensor comm is handled by through python threads, see information on
            ...
            #use the IMU simulation...
            self.imu = imuSim(simInit[0],simInit[1],simInit[0],simInit[1],simInit[2],sim,bias=True)


    def _getIMUGPS(self,gravConst = 0):
            ...
            #grab the data, assume imu data is structured11111111 as (ax,ay,az,gx,gy,gz,mx,my,mz)
            ...
            if self.imu is None:
                ...
                data = self.imu.sample()
                self.gpsPos = np.array([[float(data[9])],[float(data[10])],[float(data[11])]])
                self.gpsVelNed = float(data[12])*np.array([[1],[1],[1]])*(1000/3600.)

            #unpack data
            # print(data)
            self.accelMeas = np.asarray([[data[0]],[data[1]],[data[2]]])
            # print(self.accelMeas)
            # print(self.biasA)
            self.rotMeas = np.asarray([data[3],data[4],data[5]])
            self.magMeas = np.asarray([data[6],data[7],data[8]])

            #scale accel by g
            self.accelMeas *= gravConst
    ...
    def startup(self):
        ...
        if self.imu is not None:
            if self.imu.sim == 'lin' or self.imu.sim == 'stat':
        ...
        while(not sampleCount == 3000):
            self._getIMUGPS()
            biasEstimatorA += self.accelMeas/3000.
            biasEstimatorG += self.rotMeas/3000.
            biasEstimatorM += self.magMeas/3000.
            sampleCount += 1

        #init bias values
        self.biasG = np.asarray(biasEstimatorG)
        self.biasA = np.asarray(biasEstimatorA)
        print 'estimated gyro bias: ',self.biasG
        print 'estimated Accel bias: ',self.biasA
        # print self.biasA.shape
    ...
    def _buildStateTranMatrix(self):
        #need to recalulate the RE and RN with the newest corrected latitude, longitude, and altitude
        self.prevPHI = self.PHI

        #get submatrices
        f11 = -(self.earthRotINNed + self.rotEarthandNedSkew)

        f12 = np.array([[0,-1/(self.RE + self.alti),0],
                             [1/(self.RE + self.alti),0,0],
                             [0,np.tan(self.lat)/(self.RE + self.alti),0]])

        f13 = np.array([[self.omegaE*np.sin(self.lat),0,self.velNed[1]/(self.RE + self.alti)**2],
        ...
        f21 = -1*np.array([[0,-np.dot(self.rotB2N[2,:],self.accelMeas),np.dot(self.rotB2N[1,:],self.accelMeas)],
        ...
        f22 = np.array([[self.velNed[2][0]/(self.RN + self.alti),-2*np.tan(self.lat)*self.velNed[1][0]/(self.RE + self.alti) - 2*self.omegaE*np.sin(self.lat),self.velNed[0][0]/(self.RN + self.alti)],
        ...
        f23 = np.array([[-(((self.velNed[1][0]**2)*((1/np.cos(self.lat))**2))/(self.RE + self.alti)) - 2*self.velNed[1][0]*self.omegaE*np.cos(self.lat),0,((((self.velNed[1][0]**2)*((np.tan(self.lat))))/((self.RE + self.alti)**2))) - ((self.velNed[0][0]*self.velNed[2][0])/(((self.RN + self.alti)**2)))],
        ...
        f32 = np.array([[1/(self.RN + self.alti),0,0],
                             [0,1/((self.RE + self.alti)*np.cos(self.lat)),0],
                             [0,0,-1]])

        f33 = np.array([[0,0,self.velNed[0][0]/(self.RN + self.alti)**2],
        ...
        self.PHI = np.eye(15) + (np.vstack((np.hstack((f11,f12,f13,np.zeros((3,3)),self.rotB2N)),
                                          np.hstack((f21,f22,f23,self.rotB2N,np.zeros((3,3)))),
                                          np.hstack((np.zeros((3,3)),f32,f33,np.zeros((3,3)),np.zeros((3,3)))),
